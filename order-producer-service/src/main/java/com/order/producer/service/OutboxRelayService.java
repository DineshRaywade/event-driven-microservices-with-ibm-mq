...
public class OutboxRelayService {
    ...
    // Runs every 10 seconds — picks up PENDING events and publishes to MQ
    ...
    public void relay() {
        ...
        for (OutboxEvent event : pending) {
            try {
                ...
                OrderMessage order =
                        objectMapper.readValue(event.getPayload(), OrderMessage.class);
                ...
                log.info("Outbox relay: published orderId={}",
                        event.getOrderId());

            } catch (Exception e) {

                log.error("Outbox relay FAILED for orderId={}: {}",
                        event.getOrderId(),
                        e.getMessage());
